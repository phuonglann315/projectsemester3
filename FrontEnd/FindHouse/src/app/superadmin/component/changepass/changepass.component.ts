...
export class ChangePassComponent implements OnInit {
  ...
  constructor(private formBuilder: FormBuilder, private accSer: AccountService, private loSer: LoginService, private check: Check, private messageService: MessageService, private confirmationService: ConfirmationService) {
        ...
        if (token == "Invaid creadentials") {
          ...
          if (this.errorPass == "" && this.errorNewPass == "" && this.errorReNewPass == "") {
            this.confirmationService.confirm({
              message: 'Are you sure change password?',
              header: 'Confirmation',
              ...
              accept: () => {
                ...
                this.accSer.changePassword(lo).then(
                  res => {
                    ...
                    if (re.result) {
                      this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Change Success' });
                      this.changePassForm = this.formBuilder.group({
                        password: "",
                        newPassword: "",
                        confirmPassword: ''
